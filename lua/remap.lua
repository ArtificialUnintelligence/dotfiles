...
nnoremap("<leader>fe", "<cmd>lua require('telescope').extensions.file_browser.file_browser{ follow_symlinks = true }<cr>")
...
nnoremap("<leader>js", ':TermExec cmd="export DB_LOCATION=C:/Users/FeketeA/dbconfig/dbconn_noprod_jboss.properties;rm -rf ../wildfly/standalone/deployments/*;rm -rf ../wildfly/standalone/tmp/*;mvn clean package -Dmaven.test.skip;FILENAME=$(basename target/*.war);unzip -q target/$FILENAME -d ../wildfly/standalone/deployments/$FILENAME/;touch ../wildfly/standalone/deployments/$FILENAME.dodeploy;../wildfly/bin/standalone.sh"<cr>')
